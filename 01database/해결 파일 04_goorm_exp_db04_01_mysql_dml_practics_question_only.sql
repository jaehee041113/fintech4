...
-- 틀림 36. 2025-03-01 이후의 데이터만 조회하세요.
select * from stock_prices_2025 where Date>2025-03-01;

-- 틀림 37. 2025-03-01 이전의 데이터만 조회하세요.

-- 38. 종가 기준 내림차순, 같으면 거래량 내림차순으로 50행 조회하세요.
select * from stock_prices_2025 order by Close desc, Volume asc limit 50;
...
SELECT *
FROM stock_prices_2025 s  inner JOIN korea_stock_company k ON s.종목코드 = k.종목코드
limit 20;

-- 44. (LEFT) korea_stock_company의 모든 컬럼과 2025년 가격을 30행 조회하세요.
SELECT *
FROM korea_stock_company k left JOIN stock_prices_2025 s ON k.종목코드 = s.종목코드
limit 30;

-- 45. (RIGHT) korea_stock_company를 기준으로 2025 가격을 조인하고 30행 조회하세요.
SELECT *
FROM stock_prices_2025 s right JOIN korea_stock_company k ON s.종목코드 = k.종목코드
limit 30;

-- 틀림 46. (INNER) 업종에 '반도체'가 포함된 기업의 2025 데이터를 조회하세요.
SELECT *
FROM stock_prices_2025 s inner JOIN korea_stock_company k ON s.종목코드 = k.종목코드
where k.업종="%반도체%";
...
-- 틀림 47. (LEFT) korea_stock_company 테이블과 2025년 주가 테이블을 조인하고 지역명에 "서울"이 들어간 데이터를 조회하세요. 
SELECT *
FROM korea_stock_company k left JOIN stock_prices_2025 s ON k.종목코드 = s.종목코드
where k.지역="%서울%";

-- 틀림 48. (RIGHT) 대표자명에 '김'이 포함된 회사(가격 유무 무관)를 조회하세요.
SELECT *
FROM korea_stock_company k right JOIN stock_prices_2025 s ON k.종목코드 = s.종목코드
where k.대표자명="%김%";

-- 틀림 49. (LEFT) korea_stock_company와 2025 데이터를 조인하고 상장일이 '2010-01-01' 이후인 회사를 찾으세요.
 
-- 50. (INNER) 홈페이지가 공백이 아닌 기업의 2025 데이터를 조회하세요. (함수 없이)
SELECT *
FROM korea_stock_company k inner JOIN stock_prices_2025 s ON k.종목코드 = s.종목코드
where k.홈페이지!="% %";
